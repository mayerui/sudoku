...
void CScene::play()
{
    ...
    while (1)
    {
        ...
        else if (key == keyMap->LEFT3) {
            _cur_point.x = (_cur_point.x - 3) < 0 ? 0 : _cur_point.x - 3;
            show();
        } 
        else if (key == keyMap->RIGHT3) {
            _cur_point.x = (_cur_point.x + 3) > 8 ? 8 : _cur_point.x + 3;
            show();
        } 
        else if (key == keyMap->DOWN3) {
            _cur_point.y = (_cur_point.y + 3) > 8 ? 8 : _cur_point.y + 3;
            show();
        } 
        else if (key == keyMap->UP3) {
            _cur_point.y = (_cur_point.y - 3) < 0 ? 0 : _cur_point.y - 3;
            show();
        } 
        else if (key == keyMap->HOME) {
            _cur_point.x = 0;
            show();
        } 
        else if (key == keyMap->END) {
          _cur_point.x += 8;
          show();
        } 
        else if (key == keyMap->PGUP) {
          _cur_point.y = 0;
          show();
        } 
        else if (key == keyMap->PGDN) {
          _cur_point.y += 8;
          show();
        }
